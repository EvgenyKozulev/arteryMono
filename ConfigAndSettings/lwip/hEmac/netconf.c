...
/* Private variables ---------------------------------------------------------*/
struct netif netif;
TaskHandle_t link_status_handle;
TaskHandle_t lwip_periodic_handle;
volatile uint32_t tcp_timer = 0;
volatile uint32_t arp_timer = 0;
bool netif_is_up = false;
struct udp_pcb *pcb;
static uint8_t mac_address[MAC_ADDR_LENGTH] = {0x84, 0x11, 0xC2, 0x40, 0x02, 0xA4};
#if LWIP_DHCP
...
//	netif_set_link_callback(&netif, ethernetif_update_config);
  ...
  You can peek in the netif->dhcp struct for the actual DHCP status.*/
	...
	xTaskCreate((TaskFunction_t)ethernetif_set_link, "ethernetif_set_link", 512, NULL, 2, &link_status_handle);
//	xTaskCreate((TaskFunction_t)lwip_periodic_handler, "lwip_periodic_handle", 512, NULL, 5, NULL);
}

char *netconf_get_ip_string()
{
	return ipaddr_ntoa(&netif.ip_addr);
}
/**
 ...
 * @retval none
 */
void lwip_periodic_handler(void const *argument)
{
	for (;;) {

		volatile uint32_t localtime = xTaskGetTickCount();
